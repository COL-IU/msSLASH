...
int main(int argc, char *argv[])
{
    ...
    const int L = 1; //hash table
    int m = 12; //hash functions
    const int m_decay_cycle = 40;

    constexpr int iterations = 10; //used to be 80
    ...
    cout << "hash funcs#: " << m << endl;
    cout << "decay cycle: " << m_decay_cycle << endl;
    ...
    int ii = 0;
    map<int, vector<int>> lsh_table;
    ofstream out;
    bitset<iterations> foo;
    foo.set();

    const int iteration_max = 200; 
    //while(foo.any())
    while(foo.any() && ii < iteration_max)
    {
        cout << "ITERATION " << ++ii << endl;
        cout << "SPECTRA SIZE " << spectra_unknown.size() << endl;
        start_time = chrono::high_resolution_clock::now();

        if(0 == ii % m_decay_cycle)
            m -= 1;

        vector<vector<vector<double>>> hash_functions = Nearest_Neighbor::getLHashFunctions(L, m, randomSize, min_random, max_random, false, false);
        vector<vector<double>> hash_function = hash_functions.front();
        end_time = chrono::high_resolution_clock::now();

        elapsed_read = chrono::duration_cast<std::chrono::duration<double>>(end_time - start_time).count();
        cout << " lsh construction time:\t" << elapsed_read << endl;


        start_time = chrono::high_resolution_clock::now();
        //unordered_map<int, vector<int>> lsh_table;
        lsh_table.clear();
        string out_file = "./out/adult_liver_new_step" + to_string(iterations) + ".mgf"; 
        out.clear();
        out.open(out_file);
        for (size_t i = 0; i < spectra_unknown.size(); ++i)
        {
            const auto & spectrum = spectra_unknown.at(i);


#ifdef WRITE_IO
            out << "spectrum id: " << i << endl; 
            out << "BEGIN IONS" << endl;
            out << "PEPMASS=" << spectrum.precursor_mz << endl;
            out << "CHARGE=" << spectrum.charge<<"+" << endl;
            for (const auto & peak : spectrum.peaks)
                out << peak.first << "\t" << peak.second << endl;
            out << "END IONS" << endl;
            out << endl << endl;
#endif
            ...
            int hash_key = Nearest_Neighbor::hash_custom(hash_function, spectrum.peaks);
            lsh_table[hash_key].push_back(i);


#ifdef DEBUG
            if(i < 5)
            {
                cout << "spectrum id: " << i << endl;
                cout << "hash dec id:\t" << hash_key << endl;
                cout << "hash bin id:\t" << bitset<8>(hash_key).to_string() << endl;
                cout << endl;
            }
#endif
        }
        ...
        end_time = chrono::high_resolution_clock::now();
        elapsed_read = chrono::duration_cast<std::chrono::duration<double>>(end_time - start_time).count();
        cout << " lsh hash time:\t" << elapsed_read << endl;
        
        
        string cluster_file = "./out/cluster_step" + to_string(iterations) + ".txt.cos" + to_string(cos_threshold)+"pre"+to_string(precision);
        out.clear();
        out.open(cluster_file);

        start_time = chrono::high_resolution_clock::now();

        bool clustered = false;
        for (auto & entry : lsh_table)
        {
            out << "hash_key: " << entry.first << endl;
            out << "hash_key: " << bitset<8>(entry.first).to_string() << endl;
            for (const auto & id : entry.second)
            {
                const auto & spectrum = spectra_unknown.at(id);
                out << id << "(" +  to_string(spectrum.precursor_mz) + ")\t" ;
            }
            out << endl;

            double max_cos_sim = -1; 
            int s1_id, s2_id;
            int max_i, max_j;
            ...
            do
            {
                ...
                {
                    const auto & curSpectrum = spectra_unknown.at(entry.second[i]); 
                    for (size_t j = i + 1; j < len; ++j)
                    {
                        const auto & candSpectrum = spectra_unknown.at(entry.second[j]);
                        ...
                        //    out << "Cluster " << entry.second[i] << " <-> " << entry.second[j] << " :" << cos_sim << endl; 
                        if (cos_sim >= cos_threshold && cos_sim > max_cos_sim)
                        {
                            max_cos_sim = cos_sim;
                            s1_id = entry.second[i], s2_id = entry.second[j];
                            max_i = i, max_j = j;
                        }
                    }
                }//for
                ...
                if (max_cos_sim >= 0)
                {
                    ...
                    entry.second[max_i] = index;
                    entry.second.erase(entry.second.begin() + max_j);
                    inCluster.insert(s1_id);
                    inCluster.insert(s2_id);
                    out << "Cluster " << s1_id << " & " << s2_id << " -> " + to_string(index) +" : " << max_cos_sim << endl;
        ...
        if (clustered)
            foo[ii%iterations] = 1;//set the bit, find a cluster
        else 
            foo[ii%iterations] = 0;//unset the bit, no cluster

        out.close();
        end_time = chrono::high_resolution_clock::now();
        elapsed_read = chrono::duration_cast<std::chrono::duration<double>>(end_time - start_time).count();
        cout << "cluster time:\t" << elapsed_read << endl;
    }

    cout << "total steps:\t" << ii << endl;
    cout << "write cluster to files" << endl;
    string res_file = "./out/cluster-final-step" + to_string(iterations) + ".txt.cos" + to_string(cos_threshold)+"pre"+to_string(precision);
    out.clear();
    out.open(res_file);
    ...
    int cluster_cnt = 0;
    start_time = chrono::high_resolution_clock::now();
    for (const auto & entry : lsh_table)
    {
        vector<int> sets = entry.second;
        for (const auto & c : sets)
        {
            //out << "cluster-id " << cluster_cnt << endl;
            out << cluster_cnt << "\t";
            if (cluster.count(c))
            {
                ...
                /*
                out << c << "[ ";
                for (const auto & ele : cluster[c])
                    //out << ele << " ";
                    out << relations[ele] << "(" << spectra_unknown[ele].title << ") ";
                out << " ] " ;
                */
                ...
                out << spectra_unknown[c].title << ".dta" ;
                
                //out << relations[c] << "(" << spectra_unknown[c].title << ") ";
